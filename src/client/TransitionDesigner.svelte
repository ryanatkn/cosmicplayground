...
<script>
  ...
  const transition = (node, {
    ...
    return {
      delay,
      duration,
      css: (t, u) => `transform: ${computeRotation(t, rotate3dX1, rotate3dX2, rotate3dY1, rotate3dY2, rotate3dZ1, rotate3dZ2, rotate3dDeg1, rotate3dDeg2)};`
    }
  };

  const computeRotation = (t, x1, x2, y1, y2, z1, z2, deg1, deg2) => {
    return `rotate3d(${mix(x1, x2, t)}, ${mix(y1, y2, t)}, ${mix(z1, z2, t)}, ${mix(deg1, deg2, t)}deg)`;
  }

  const timeSliderWidth = 300;
  const timeSliderBorderWidth = 3;
  const timeIndicatorSize = 20;
  const timeTransition = (node, {
    delay = 0,
    duration = 1000,
  }) => {
    return {
      delay,
      duration,
      css: (t, u) => `transform: translate3d(${(timeSliderWidth - timeIndicatorSize) * t}px, 0, 0);`
    }
  };
...
<section style="height: 100px; display: flex; align-items: center; justify-content: center;">
  <img
    src={imgSrc}
    alt="styler target"
    style="width: 162px; height: 100px; transform: {rotate3d1}; margin-right: 50px;"
    class="pixelated"
  />
  <img
    src={imgSrc}
...
<section style="height: 200px; display: flex; align-items: center; justify-content: center;">
...
<section class="controls">
  ...
  <section>
    <pre>{rotate3d1}</pre>
    <label>
      <input type="range" bind:value={rotate3dX1} min={rotateMin} max={rotateMax} step={rotateStep}/>
      {rotate3dX1} <small>x1</small>
    </label>
    <label>
      <input type="range" bind:value={rotate3dY1} min={rotateMin} max={rotateMax} step={rotateStep}/>
      {rotate3dY1} <small>y1</small>
    </label>
    <label>
      <input type="range" bind:value={rotate3dZ1} min={rotateMin} max={rotateMax} step={rotateStep}/>
      {rotate3dZ1} <small>z1</small>
    </label>
    <label>
      <input type="range" bind:value={rotate3dDeg1} min={-360} max={360} step={1}/>
      {rotate3dDeg1} <small>deg1</small>
    </label>
  </section>
  ...
  <section>
    <pre>{rotate3d2}</pre>
    <label>
      <input type="range" bind:value={rotate3dX2} min={rotateMin} max={rotateMax} step={rotateStep}/>
      {rotate3dX2} <small>x2</small>
    </label>
    <label>
      <input type="range" bind:value={rotate3dY2} min={rotateMin} max={rotateMax} step={rotateStep}/>
      {rotate3dY2} <small>y2</small>
    </label>
    <label>
      <input type="range" bind:value={rotate3dZ2} min={rotateMin} max={rotateMax} step={rotateStep}/>
      {rotate3dZ2} <small>z2</small>
    </label>
    <label>
      <input type="range" bind:value={rotate3dDeg2} min={-360} max={360} step={1}/>
      {rotate3dDeg2} <small>deg2</small>
    </label>
  </section>
